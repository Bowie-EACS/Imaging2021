...
while filename_valid == False:# 
    ...
    print("Which image would you like to edit?")#Ask what they would like to edit
...
class cus_window():
    ...
    (image_height, image_width, image_channels), numpy.uint8)#Defines the color paper dimentions
        ...
        #The RGB values
        if irgb == None:
            cv2.createTrackbar("R",self.name_window,random.randint(0,255),255,lambda x:self.update_colors())
            cv2.createTrackbar("G",self.name_window,random.randint(0,255),255,lambda x:self.update_colors())
            cv2.createTrackbar("B",self.name_window,random.randint(0,255),255,lambda x:self.update_colors())
        else:
            cv2.createTrackbar("R",self.name_window,irgb[0],255,lambda x:self.update_colors())
            cv2.createTrackbar("G",self.name_window,irgb[1],255,lambda x:self.update_colors())
            cv2.createTrackbar("B",self.name_window,irgb[2],255,lambda x:self.update_colors())
